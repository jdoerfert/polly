...
static cl::opt<bool> Aggressive("polly-prop-aggressive", cl::desc(""), cl::Hidden,
...
unsigned ExpressionPropagation::computeNumAccessedStreams(
  ...
  std::sort(Edges.begin(), Edges.end(),
            [](const RAWEdge *E0, const RAWEdge *E1) {
              return E0->Trg->MA->AccNo < E1->Trg->MA->AccNo;
            });

  SmallPtrSet<MemoryAccess *, 8> TrgMAs;
  for (auto *Edge : Edges) {
    //errs() << "EdgeNo: " << Edge->Trg->MA->AccNo << "\n";
...
unsigned ExpressionPropagation::computeNumAccessedStreams(
  ...
  for (unsigned u = 0; u < Nodes.size() && NumAccessStreams + NonAffineAccs <= Limit; u++) {
    ...
    for (auto *MA : *Stmt) {
      ...
      NextCellMap = isl_map_lower_bound_si(NextCellMap, isl_dim_out, NumDims -1, -3);
      NextCellMap = isl_map_upper_bound_si(NextCellMap, isl_dim_out, NumDims -1, 3);
      //(errs() << "NextCellMap: " << NextCellMap << "\n");
...
void ExpressionPropagation::createDependenceGraph(Scop &S) {
  ...
  for (auto &It : DefNodeGroupMap) {
    if (It.second->isEmpty() || It.second->IsLiveOut || !It.second->IsAffine)
      continue;
...
static bool containsUDiv(MemoryAccess *MA) {
  auto *Stmt = MA->getStatement();
  if (MA->getNumSubscripts() < 1)
    return false;
  isl_map *NextItMap = isl_map_lex_lt(Stmt->getDomainSpace());
  for (unsigned d = 0; d + 1 < Stmt->getNumIterators(); d++)
    NextItMap = isl_map_equate(NextItMap, isl_dim_in, d, isl_dim_out, d);
  NextItMap = isl_map_lexmin(NextItMap);
  isl_map *AccRel = MA->getAccessRelation();
  //errs() << "AccRel: " << AccRel << "\n";
  AccRel = isl_map_project_out(AccRel, isl_dim_out, 0,
                               isl_map_dim(AccRel, isl_dim_out) - 1);
  //errs() << "AccRel: " << AccRel << "\n";
  NextItMap = isl_map_apply_domain(NextItMap, isl_map_copy(AccRel));
  NextItMap = isl_map_apply_range(NextItMap, AccRel);
  //errs() << "NextIt: " << NextItMap << "\n";
  bool IsEmpty = isl_map_is_empty(NextItMap);
  isl_map_free(NextItMap);
  return IsEmpty;
};

bool ExpressionPropagation::propagateEdges(DefNodeGroup &DNG, unsigned *MaxStreamsPtr) {
  ...
  for (auto *Def : DNG.Nodes) {
    ...
    if (!SrcIsVectorizable && NumNonVecDims == 0) {
      errs() << "Src is non vectorizable: " << Def->getName() << "\n";
    ...
    for (auto &EdgeIt : Def->getEdges<RAWEdge>()) {
      ...
      if (RAW->Trg->isAffine()) {
        if (!NonVecTrgGroups.count(RAW->Trg->getDefNode().Group)) {
          ...
          if (!TrgIsVectorizable) {
            errs() << "Trg is non vectorizable: " << RAW->Trg->getName() << "\n";
          }
  ...
  for (auto &TrgEdgeMapIt : TrgEdgeMap) {
    ...
    for (RAWEdge *Edge : Edges) {
      ...
      if (TrgEdgeMap.count(Src)) {
        ...
        continue;
      } else if (!Aggressive && IsExpandingEdge(*Edge)) {
  ...
  if (!Aggressive) {
    ...
    for (auto &It : PerSAITargets) {
      ...
      DEBUG(errs() << "\tCheck targets with SAI: " << It.first->getName()
                   << "\n");
      unsigned Streams = 0;
      bool Worth = isWorthRecomputing(Trgs.back(), Edges, Streams);
      MaxStreams = std::max(MaxStreams, Streams);
      if (Worth)
        continue;
      if (!DNG.IsLiveOut) {
        if (MaxStreamsPtr)
          *MaxStreamsPtr = 0;
        (errs() << "Not worth to recompute in write Stmts to "
                << It.first->getName() << "\n");
        return false;
      }
...
bool ExpressionPropagation::propagateExpressions(Scop &S) {
  ...
  while (!Worklist.empty()) {
    ...
    #if 0
    errs() << "\n\n---------- WORKLIST ---\n";
    for (auto  *DNG : WorklistSet ) {
      unsigned Ops = 0;
      std::for_each(DNG->Nodes.begin(), DNG->Nodes.end(),
                    [&](DefNode *Def) { Ops += Def->Operands.size(); });

      auto Potential = DNG->getNumArrayLoweringPotential();
      unsigned Streams = 0;
      propagateEdges(*DNG, &Streams);
      (errs() << "- POSSIBLE -  " << DNG->SAI->getName() << " [#N: " << DNG->Nodes.size()
              << "] [#E: " << DNG->numOutgoingEdges<RAWEdge>()
              << "] [Pot: " << Potential << "] [Ops: " << Ops
              << "] [Str: " << Streams << "]\n");
    }
    errs() << "\n\n-------------\n";
    #endif
    ...
    DEBUG(errs() << "\n------!!CHECK!!: " << DNG->SAI->getName()
                 << " [#N: " << DNG->Nodes.size() << "] [#E: "
                 << DNG->numOutgoingEdges<RAWEdge>() << "] [Pot: " << Potential
                 << "] [Ops: " << Ops << "] [Str: " << Streams << "]\n");
...
// True  => D1, D0
// False => D0, D1
bool GroupNodeCompare::compare(DefNodeGroup *DNG0, DefNodeGroup *DNG1) const {
  if (Aggressive)
  ...
  unsigned MaxStreams0 = 0, MaxStreams1 = 0;
  DNG0->Nodes.front()->ExprProp.propagateEdges(*DNG0, &MaxStreams0);
  DNG0->Nodes.front()->ExprProp.propagateEdges(*DNG1, &MaxStreams1);
  errs() << "MS0: " << MaxStreams0 << "  MS1: " << MaxStreams1 << "\n";
  if (MaxStreams0 != MaxStreams1)
    return MaxStreams0 < MaxStreams1;
  ...
  if (DNG0->SAI->getNumberOfDimensions() != DNG1->SAI->getNumberOfDimensions())
