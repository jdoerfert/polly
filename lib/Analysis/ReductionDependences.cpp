//===- ReductionDependences.cpp - Calculate reduction dependences --------===//
//
...
ReductionDependences::ReductionDependences() : Dependences(ID) {}

void ReductionDependences::collectInfo(Scop &S, isl_union_map **Read,
                                       isl_union_map **Write,
...
void ReductionDependences::calculateDependences(Scop &S) {
...
char ReductionDependences::ID = 0;

void ReductionDependences::getAnalysisUsage(AnalysisUsage &AU) const {
...
Pass *polly::createReductionDependencesPass() {
  return new ReductionDependences();
}

INITIALIZE_PASS_BEGIN(ReductionDependences, "polly-reduction-dependences",
                      "Polly - Calculate reduction dependences", false, false);
INITIALIZE_PASS_DEPENDENCY(LoopInfo);
INITIALIZE_PASS_DEPENDENCY(Dependences);
INITIALIZE_AG_DEPENDENCY(ReductionInfo);
INITIALIZE_PASS_END(ReductionDependences, "polly-reduction-dependences",
                    "Polly - Calculate reduction dependences", false, false)
