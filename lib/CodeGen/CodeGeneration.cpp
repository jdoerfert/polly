...
static bool CodeGen(Scop &S, IslAstInfo &AI, LoopInfo &LI, DominatorTree &DT,
                    ScalarEvolution &SE, RegionInfo &RI, ScopDetection *SD) {
  ...
  simplifyRegion(R, &DT, &LI, &RI, SD);
  assert(R->isSimple() || containsEntry(*R));
  BasicBlock *EnteringBB = S.getEnteringBlock();
  if (!EnteringBB) {
    assert(containsEntry(*R));
    EnteringBB = splitEntryBlockForAlloca(&F.getEntryBlock(), &DT, &LI, &RI);
  }
  ...
  if (PerfMonitoring) {
    ...
    if (ExitBlock && ExitBlock->getUniqueSuccessor()) {
      ...
      for (auto &BB : F) {
        ...
        if (isa<UnreachableInst>(TI)) {
          assert(BB.size() > 1 && isa<CallInst>(TI->getPrevNode()) &&
                 cast<CallInst>(TI->getPrevNode())->doesNotReturn());
          IP = TI->getPrevNode();
        }
        P.insertRegionEnd(IP);
      }
    }
  }
  ...
  if (!NodeBuilder.preloadInvariantLoads()) {
    ...
    if (PollyDebugPrinting) {
      RuntimeDebugBuilder::createCPUPrinter(Builder, "Enter ", R->getNameStr(),
                                            " in ", F.getName(), "\n");
      for (auto *P : S.getParameters()) {
        auto Id = S.getIdForParam(P);
        RuntimeDebugBuilder::createCPUPrinter(
            Builder, "  Param  ", Id.get_name(), ": ",
            NodeBuilder.generateSCEV(P), "\n");
      }
    }
...
public:
  ...
  bool runOnScop(Scop &S) override {
    ...
    return CodeGen(S, *AI, *LI, *DT, *SE, *RI, SD);
  }
...
PreservedAnalyses CodeGenerationPass::run(Scop &S, ScopAnalysisManager &SAM,
  ...
  // TODO Provide scop detection!
  if (CodeGen(S, AI, AR.LI, AR.DT, AR.SE, AR.RI, nullptr)) {
    U.invalidateScop(S);
